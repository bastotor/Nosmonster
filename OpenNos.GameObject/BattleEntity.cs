    ...
    public class BattleEntity
    {
        ...
        public List<BattleEntity> TargettedByMonstersList(bool teamCheck)
        {
            ...
            else
            {
                ...
                {
                    targettedByMonsters = MapInstance?.Monsters.Where(s => s?.Target?.MapEntityId == MapEntityId && s.Target.EntityType == EntityType).Select(s => s.BattleEntity).ToList();//basto crash

                    if (Character != null)
                    {
                        Character.Mates.Where(s => s.IsTeamMember).ToList().ForEach(m => targettedByMonsters.AddRange(MapInstance?.Monsters.Where(s => s?.Target?.MapEntityId == m.BattleEntity.MapEntityId && s.Target.EntityType == m.BattleEntity.EntityType).Select(s => s.BattleEntity).ToList())); // Kentao CRASH
                    }
                }
                ...
                if (teamCheck)
                {
                    switch (EntityType)
                    {
                        case EntityType.Player:
                            return (1 + Character.Mates.Count(m => m.IsTeamMember || m.IsTemporalMate)) * 100;

                        case EntityType.Mate:
                            return (1 + Mate.Owner.Mates.Count(m => m.IsTeamMember || m.IsTemporalMate)) * 100;
                    }
                }
                else
                {
        ...
        public List<int[]> GetBuffInRW(CardType type, byte subtype, int secondData = -1)
        {
            ...
            List<BCard> Bcards = BCards.Where(s => s?.Type.Equals((byte)type) == true && s.SubType.Equals((byte)(subtype / 10)) && (secondData == -1 || s.SecondData == secondData));
            foreach (BCard entry in Bcards)
            {
                values.Add(new int[] { entry.FirstData, entry.SecondData, entry.ThirdData });

                ThirdData += entry.ThirdData;
            }
            //values.Add(ThirdData);
            return values;
        }
        ...
        {
            int GetShellArmorEffectValue(ShellArmorEffectType effectType)//basto
            {
                return Character.ShellEffectArmor?.Where(s => s.Effect == (byte)effectType).FirstOrDefault()?.Value ??
                       0;
            }
            ...
            if (indicator.Card != null)
            {
                ...
                if (Character != null && indicator.Card.BuffType == BuffType.Bad)//basto
                {
                    int ReduceBadEffect = GetShellArmorEffectValue(ShellArmorEffectType.ReducedAllNegativeEffect);
                    if (ReduceBadEffect != 0)
                    {
                        if (ServerManager.RandomNumber() <= ReduceBadEffect)
                        {
                            Character.Session.SendPacket(Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("NEGATIVE_EFFECT"), indicator.Card.Name), 20));
                            return;
                        }
        ...
        public bool HasBCard(CardType type, byte subtype)//basto
        {
            try
            {
                return BCards.Any(b =>
                            ...
                            case 620:
                                {
                                    if (Character.Session != null && Character.SavedLocation != null && indicator.Sender?.Character?.CharacterId == Character.CharacterId)
                                    {
                                        ...
                                        if (skill != null)
                                        {
                                            short cooldown = 0; // 60 seconds * 10
        ...
        public double HPLoad()
        {
            ...
            if (Character != null)
            {
                ...
                var hpPrestige = Character.Prestige * Level * 10;
                MaxHp = (int)((CharacterHelper.HPData[(byte)Character.Class, Level] + hp + hpPrestige + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPIncreased)[0] + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPMPIncreased)[0]) * multiplicator);
            }
            else
            {
                MaxHp = HpMax;
        ...
        public double MPLoad()
        {
            ...
            if (Character != null)
            {
                ...
                var mpPrestige = Character.Prestige * Level * 10;
                MaxMp = (int)((CharacterHelper.MPData[(byte)Character.Class, Level] + mp + mpPrestige + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumMPIncreased)[0] + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPMPIncreased)[0]) * multiplicator);
            }
            else
            {
                MaxMp = MpMax;
