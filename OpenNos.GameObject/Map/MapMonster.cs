...
namespace OpenNos.GameObject
{
    public class MapMonster : MapMonsterDTO
    {
        ...
        public BattleEntity Target { get; set; } // ici
        ...
        {
            if (!IsHostile && !MonsterHelper.IsAutoTarget(MonsterVNum) || Target != null)
            {
                return;
            }
            ...
            if (target.Character != null && MonsterHelper.IsAutoTarget(MonsterVNum))
            {
                var mylist = new List<short> { 200, 201, 202, 203 };
                var follow = target.Character.BattleEntity?.TargettedByMonstersList(false).Any(s =>
                    mylist.Contains(s.MapMonster.MonsterVNum)) ?? false;
        ...
        private void TargetHit(BattleEntity target, NpcMonsterSkill npcMonsterSkill)
        {
            if (Monster != null && !HasBuff(CardType.SpecialAttack, (byte)AdditionalTypes.SpecialAttack.NoAttack))
            {
                ...
                Observable.Timer(TimeSpan.FromMilliseconds(castTime)).Subscribe(obs =>
                {
                    ...
                    int damage = DamageHelper.Instance.CalculateDamage(new BattleEntity(this), //kentao crash , peut être crash ici aussi
                    ...
                    && (skillTargetRange == 0 || Map.GetDistance(new MapCell { X = MapX, Y = MapY }, new MapCell { X = target.PositionX, Y = target.PositionY }) <= skillTargetRange))
                {
                    ...
                    if (target.Hp <= 0)
                    {
                        if (target.Character != null)
                        {
                            ...
                            {
                                if (target?.Character?.Session.CurrentMapInstance?.MapInstanceType == MapInstanceType.BattleRoyalInstance)
                                { // item save battle royal
                                    int vnum = BattleROYA2.ItemSaved;
                                    if (target.Character.Inventory.CountItem(vnum) >= 1)
                                    {
                                        target.Character.Inventory.RemoveItemAmount(vnum, 1);
                                        Observable.Timer(TimeSpan.FromMilliseconds(1000)).Subscribe(o =>
                                        ServerManager.Instance.AskRevive(target.Character.CharacterId));
                                    }
                                    else
                                    {
                                        if (target.Character.IsBattleRoyalLevel == true)
                                        {
                                            target.Character.Session.VerifyStuff();
                                            target.Character.Session.BackupLevelNormal();
                                        }
                                        Observable.Timer(TimeSpan.FromSeconds(1)).Subscribe(o =>
                                        {
                                            ServerManager.Instance.ChangeMap(target.Character.CharacterId, target.Character.MapId,
                                                                target.Character.MapX, target.Character.MapY);
                                        });
