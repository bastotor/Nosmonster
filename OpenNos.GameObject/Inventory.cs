...
namespace OpenNos.GameObject
{
    public class Inventory : ThreadSafeSortedList<Guid, ItemInstance>
    {
        ...
        private const short DEFAULT_BACKPACK_SIZE = 255;

        private const short MAX_ITEM_AMOUNT = 32000;
                ...
                if (newItem.Type != InventoryType.Bazaar && (newItem.Item.Type == InventoryType.Etc || newItem.Item.Type == InventoryType.Main))
                {
                    ...
                    if (newItem.Amount <= (freeslot * MAX_ITEM_AMOUNT) + slotNotFull.Sum(s => MAX_ITEM_AMOUNT - s.Amount))
                    {
                        foreach (ItemInstance slot in slotNotFull)
                        {
                           ...
                           /* Logger.LogUserEvent("ITEM_CREATE", Owner.GenerateIdentity(), $"IIId: {slot.Id} ItemVNum: {slot.ItemVNum} Amount: {max - slot.Amount} MapId: {Owner.MapInstance?.Map.MapId} MapX: {Owner.PositionX} MapY: {Owner.PositionY}");*/ //basto
        ...
        public int BackpackSize() => DEFAULT_BACKPACK_SIZE + ((Owner.HaveBackpack() ? 1 : 0) * 12) + ((Owner.HaveNewBackpack() ? 1 : 0) * 60);
            ...
            if (inventory != null)
            {
                ...
                if (inv == null)
                {
                    if (IsBattleRoyal == true)
                    {
                        ...
                        Session.SendPacket("say 1 -1 10 Tu a été KIck du Battle Royal , car ton inventaire est full .");
                    }
