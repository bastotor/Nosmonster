...
namespace OpenNos.GameObject.Networking
{
    public class ServerManager : BroadcastableBase
    {
        ...
        public ConcurrentDictionary<FishingSpotsDto, List<FishInfoDto>> FishingSpots { get; set; }
        ...
        public MapInstance ArenaInstance2 { get; private set; }
        ...
        public List<CharacterTitleDTO> CharacterTitle { get; private set; }
        ...
        public List<FamilyDTO> TopFamily { get; set; }
        ...
        public List<ConcurrentBag<RainbowBattleMember>> RainbowBattleTeams { get; set; } = new List<ConcurrentBag<RainbowBattleMember>>();
        ...
        public List<FishInfoDto> GetFishingSpot(short mapId, short mapX, short mapY)
        {
            var kvp = FishingSpots.FirstOrDefault(s => s.Key.MapId == mapId && s.Key.MapX == mapX && s.Key.MapY == mapY);

            if (kvp.Equals(default(KeyValuePair<FishingSpotsDto, List<FishInfoDto>>)))
            {
                return null;
            }
        ...
        public void AskPvpRevive(long characterId)
        {
            ...
            if (session?.HasSelectedCharacter == true)
            {
                ...
                switch (session.CurrentMapInstance.MapInstanceType)
                {
                    ...
                    default:
                        if (session.CurrentMapInstance == ArenaInstance || session.CurrentMapInstance == FamilyArenaInstance || session.CurrentMapInstance == ArenaInstance2)
                        {
                ...
                switch (session.CurrentMapInstance.MapInstanceType)
                {
                    case MapInstanceType.BaseMapInstance:
                        if (session.Character.Level > 20 && ChannelId != 51)
                        {
                            ...
                            if (session.Character.HeroXp > 0 && session.CurrentMapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act61 || s.MapTypeId == (short)MapTypeEnum.Act61a || s.MapTypeId == (short)MapTypeEnum.Act61d || s.MapTypeId == (short)MapTypeEnum.Act71))
                            {
                                session.Character.HeroXp -= 60000;
                                session.SendPacket(session.Character.GenerateSay(Language.Instance.GetMessageFromKey("LOSE_HERO_XP"), 11));
                            }
        ...
        public void ChangeMapInstance(long characterId, Guid mapInstanceId, int? mapX = null, int? mapY = null, bool noAggroLoss = false)
        {
            ...
            if (session?.Character != null && !session.Character.IsChangingMapInstance)
            {
                ...
                try
                {
                    ...
                    if (session.CurrentMapInstance.MapInstanceType == MapInstanceType.BattleRoyalInstance && gotoMapInstance.MapInstanceType != MapInstanceType.BattleRoyalInstance)
                    {
                    ...
                    else if (session.Character.Reputation < gotoMapInstance.MinReputation) //Kentao reput.
                    {
                        session.SendPacket(UserInterfaceHelper.GenerateInfo(string.Format(Language.Instance.GetMessageFromKey("LOW_REQUIRED_REPUTATION"), gotoMapInstance.MinReputation)));
                        session.Character.IsChangingMapInstance = false;
                        return;
                    }
                    ...
                    Parallel.ForEach(
                        session.CurrentMapInstance.Sessions.Where(s =>
                            ...
                            s.Character.CharacterId != session.Character.CharacterId), visibleSession =>
                            {
                                ...
                                session.SendPacket(visibleSession.Character.GeneratePairy());
                            });
        ...
        public void Initialize()
        {
            Configuration = new ConfigurationObject
            {
                ...
                DoubleFairyXP = bool.Parse(ConfigurationManager.AppSettings["DoubleFairyExp"]),
                DoubleRaidBox = bool.Parse(ConfigurationManager.AppSettings["DoubleRaidBox"]),
                DobleSpUp = bool.Parse(ConfigurationManager.AppSettings["DobleSpUp"]),
                DobleEqUp = bool.Parse(ConfigurationManager.AppSettings["DobleEqUp"]),
                dobleapuestaUp = bool.Parse(ConfigurationManager.AppSettings["dobleapuestaUpEv"]),
                DoblePerfectionUp = bool.Parse(ConfigurationManager.AppSettings["DoblePerfectionUpEv"]),
                DobleExpeFamily = bool.Parse(ConfigurationManager.AppSettings["DobleExpeFamilyEv"]),
                DobleGold = bool.Parse(ConfigurationManager.AppSettings["DobleGoldEv"]),
                DobleReput = bool.Parse(ConfigurationManager.AppSettings["DobleReputEv"]),
                DobleDrop = bool.Parse(ConfigurationManager.AppSettings["DobleDropEv"]),
            ...
            Parallel.ForEach(itemPartitioner, new ParallelOptions { MaxDegreeOfParallelism = 4 }, itemDto =>
            {
                switch (itemDto.ItemType)
                {
                    ...
                    case ItemType.Title:
                        _items.Add(new TitleItem(itemDto));
                        break;
        ...
        {
            FishingSpots = new ConcurrentDictionary<FishingSpotsDto, List<FishInfoDto>>();
            var spots = DAOFactory.FishingSpotsDao.LoadAll();
            var infos = DAOFactory.FishInfoDao.LoadAll();

            foreach (var spot in spots)
            {
                var spotInfo = new List<FishInfoDto>();
                spotInfo.AddRange(infos.Where(s => s.MapId1 == spot.MapId || s.MapId2 == spot.MapId || s.MapId3 == spot.MapId));
                FishingSpots.TryAdd(spot, spotInfo);
            }

            Logger.Log.Info($"[Load]{FishingSpots.Count} Fishing Spots has been loaded");
        }
        ...
        public static int RandomNumberTitle(int min = 0, int max = 200)
        {
            lock (syncLock)
            { // synchronize
                return random.Next(min, max);
        ...
        public void RefreshRanking()
        {
            ...
            //TopFamily = DAOFactory.FamilyDAO.GetTopFamily();
        }
        ...
        public void Stat()
        {
            CommunicationServiceClient.Instance.RetrieveServerStatistics();
            foreach (string message in CommunicationServiceClient.Instance.RetrieveServerStatistics())
            {

#pragma warning disable 4014
                DiscordWebhookHelper.DiscordStatServer($"Stats:  Stats: {message} ");
            }          
        }

        public void ShoutEvent1Start()     
        {
            Shout(string.Format(Language.Instance.GetMessageFromKey("Easter Event Ongoing until April 24 at 11:59 p.m.")));

        }

        public void ShoutEvent2Start()
        {
            Shout(string.Format(Language.Instance.GetMessageFromKey("Forgot not to vote on discord in announcements for the difficulty of the server")));

        }


        public void ShoutEvent3Start()
        {
            Shout(string.Format(Language.Instance.GetMessageFromKey("Being tested on the test server: Set buffs")));

        }

        public void ShoutEvent4Start()
        {
            Shout(string.Format(Language.Instance.GetMessageFromKey("The permanent perfection reset item available on the shop =)")));
        ...
        private void LaunchEvents()
        {
            ...
            Observable.Interval(TimeSpan.FromMinutes(5)).Subscribe(x => StatServer());
            Observable.Interval(TimeSpan.FromSeconds(1)).Subscribe(x => Act4Process());
            //Observable.Interval(TimeSpan.FromMinutes(30)).Subscribe(x => ShoutEvent1());
            Observable.Interval(TimeSpan.FromMinutes(35)).Subscribe(x => ShoutEvent2());
            Observable.Interval(TimeSpan.FromMinutes(32)).Subscribe(x => ShoutEvent3());
            Observable.Interval(TimeSpan.FromMinutes(33)).Subscribe(x => ShoutEvent4());
            ...
            EventHelper.GenerateEvent(EventType.ACT5SHIP);
            EventHelper.GenerateEvent(EventType.ACT7SHIP);
        ...
        private void LoadBossEntities()
        {
            BossVNums = new List<short>
            {
                ...
                3029, // Bélial roi démon
                992, // ombre de roi des esprits
                993, // ombre de carno
                994, // ombre de bélial
                3027, //Kirollas roi des esprits /mapgold.
            };
                ...
                switch (message.Type)
                {
                    ...
                    case MessageType.Whisper:
                        ...
                        if (targetSession.Character.GmPvtBlock)
                        {
                            ...
                            && DAOFactory.AccountDAO.LoadById(DAOFactory.CharacterDAO.LoadById(message.SourceCharacterId).AccountId).Authority < AuthorityType.GameMaster)
                        {
                            if (message.DestinationCharacterId != null)
                            {
                                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage
        ...
        private void ShoutEvent1()
        {
            try
            {
                Logger.Info(Language.Instance.GetMessageFromKey("Shout_Event"));
                ShoutEvent1Start();
            }
            catch (Exception e)
            {
        ...
        private void ShoutEvent2()
        {
            try
            {
                Logger.Info(Language.Instance.GetMessageFromKey("Shout_Event2"));
                ShoutEvent2Start();
            }
            catch (Exception e)
            {
        ...
        private void ShoutEvent3()
        {
            try
            {
                Logger.Info(Language.Instance.GetMessageFromKey("Shout_Event3"));
                ShoutEvent3Start();
            }
            catch (Exception e)
            {
        ...
        private void ShoutEvent4()
        {
            try
            {
                Logger.Info(Language.Instance.GetMessageFromKey("Shout_Event4"));
                ShoutEvent4Start();
            }
            catch (Exception e)
            {
        ...
        private void StatServer()
        {
            try
            {
                Logger.Info(Language.Instance.GetMessageFromKey("STAT_SERVER"));
                Stat();
            }
            catch (Exception e)
            {
