...
namespace OpenNos.GameObject
{
    public class BCard : BCardDTO
    {
        ...
        public ClientSession sess { get; set; }
        ...
        public void ApplyBCards(BattleEntity session, BattleEntity sender, short x = 0, short y = 0, int levelUpgraded = 0)
        {
            ...
            session.BCardDisposables[skill?.SkillVNum == 1098 ? skill.SkillVNum * 1000 : BCardId] = Observable.Timer(TimeSpan.FromMilliseconds(delayTime)).Subscribe(o =>
            {
                switch ((BCardType.CardType)Type)
                {
                    case BCardType.CardType.Buff:
                        {
                            var cardId = (short)(SecondData + levelUpgraded);
                    ...
                    case BCardType.CardType.IncreaseRes:
                    ...
                    case BCardType.CardType.Morale:
                        if (SubType == (byte)AdditionalTypes.Morale.MoraleIncreased / 10)
                        {
                            if (session.Character is Character moraleChar)//test basto morale
                            {
                                ...
                                if (IsLevelScaled)
                                {
                                    ...
                                    Observable.Timer(TimeSpan.FromMilliseconds(moralecard.Duration * 100)).Subscribe(s => moraleChar.HitRate -= FirstData * moraleChar.Level);
                                }
                            }
                        }
                        break;
                    ...
                    case BCardType.CardType.SummonAct7:
                        break;

                    /*case BCardType.CardType.Reflection: // kentao crash
                        ...
                        if (SubType == (byte)AdditionalTypes.Reflection.HPIncreased / 10 && (CardId == 413 || CardId == 416)) // crash /basto
                        {
...
#warning TODO: Mana Drain
                    ...
                    case BCardType.CardType.NoDefeatAndNoDamage:
                        /*if (SubType == 3)
                        {
                            if (firstData == 0)
                            {
                                if (session.Character.HasBuff(BCardType.CardType.NoDefeatAndNoDamage, (byte)AdditionalTypes.NoDefeatAndNoDamage.TransferAttackPower))
                                {
                    ...
                    case BCardType.CardType.PvPFairyEffects:
                        break;

                    case BCardType.CardType.EffectSummon:
                        break;

                    case BCardType.CardType.Unknown1:
                        break;
                    ...
                    case BCardType.CardType.DragonSkills:
                        ...
                        break;
                    case BCardType.CardType.SecondTransformation:
                       /* Character bearStance = sender.Character != null ? sender.Character : session.Character;
                        switch (SubType)
                           {
                            case 1:
                                {
                                    if (bearStance.Morph == 42) //ici remove buff 895

                                    {
                                        bearStance.Morph = 43;
                                        bearStance.MapInstance.Broadcast(bearStance.GenerateCMode());
                                        bearStance.Session.SendPackets(bearStance.GenerateQuicklist());
                                        bearStance.Skills[1027] = new CharacterSkill
                                        {
                                            SkillVNum = 1027,
                                            CharacterId = bearStance.CharacterId
                                        };
                                    }
                                    else if (bearStance.Morph == 43)
                            ...
                            case 2:
                                {
                                    if (bearStance.Morph == 42) // remove buff 896

                                    {
                                        bearStance.Morph = 3720;
                                        bearStance.MapInstance.Broadcast(bearStance.GenerateCMode());
                                        bearStance.Session.SendPackets(bearStance.GenerateQuicklist());
                                        bearStance.Skills[1349] = new CharacterSkill
                                        {
                                            SkillVNum = 1349,
                                            CharacterId = bearStance.CharacterId
                                        };
                                    }
                                    else if (bearStance.Morph == 3720)
                    ...
                    case BCardType.CardType.WolfMaster:
                        ...
                        break;
                    case BCardType.CardType.A7Powers1: //bastoc
                        switch (SubType)
                        {
                            ...
                            case (byte)AdditionalTypes.A7Powers1.EnemyKnockedBack:
                                ...
                                if (ServerManager.RandomProbabilityCheck(FirstData)) // session.ResistForcedMovement kentao
                                {
