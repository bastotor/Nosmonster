            ...
            if (!ServerManager.Instance.StartedEvents.Contains(type))
            {
                Task.Factory.StartNew(() =>
                {
                    ...
                    switch (type)
                    {
                        ...
                        case EventType.ACT5SHIP:
                            ACT5SHIP.GenerateAct5Ship();
                            break;

                        case EventType.ACT7SHIP:
                            ACT7SHIP.GenerateAct7Ship();
                            break;
                        ...
                        case EventType.TOWERGAME: //Work Kentao towergame
                            if (ServerManager.Instance.ChannelId != 51)
                            {
                        ...
                        case EventType.BATTLEROYAL://Work Kentao BattleRoyale
                            if (ServerManager.Instance.ChannelId != 51)
                            {
                                BattleRoyale.GenerateBattleRoyale();
                            }
                            else
                            {
                                ServerManager.Instance.StartedEvents.Remove(type);
                            ...
                            switch (evt.MapInstance.MapInstanceType)
                            {
                                        ...
                                        if (evt.MapInstance.InstanceBag.EndState == 1 && evt.MapInstance.Monsters.Any(s => s.IsBoss))
                                        {
                                            Parallel.ForEach(group.Sessions.Where(s => s?.Character?.MapInstance?.Monsters.Any(e => e.IsBoss) == true), s =>
                                            {
                                                foreach (Gift gift in group.Raid.GiftItems)
                                                {
                                                    ...
                                                    if (s.Character.Level >= group.Raid.LevelMinimum && s.Character.HeroLevel >= group.Raid.HeroMinimumLevel)
                                                    {
                                    ...
                                    if (client != null)
                                    {
                                        ...
                                        if (fam != null)
                                        {
                                            ...
                                            foreach (ClientSession sess in evt.MapInstance.Sessions)
                                            {
                                                ...
                                                {
                                                    sess.Character.GiftAdd(rewardVNum, 1, forceRandom: true, minRare: 1, design: 99);
                                ...
                                case MapInstanceType.WorldBossInstance:
                                    foreach (ClientSession sess in evt.MapInstance.Sessions)
                                    {
                                        if (sess?.Character != null)
                                        {
                                            if (WorldRad.RemainingTime > 2400)
                                            {
                                                sess.Character.GetReputation(5000000);
                                                sess.Character.GiftAdd(5422, 99);
                                                sess.Character.GiftAdd(5010, 99);
                                                sess.Character.GiftAdd(1689, 99);
                                                sess.Character.Gold += 25000000;
                                                sess.SendPacket(sess.Character.GenerateGold());

                                            }
                                            else
                                            {
                                                sess.Character.GetReputation(10000000); //wb reputation
                                                sess.Character.GiftAdd(5422, 150);
                                                sess.Character.GiftAdd(5010, 150);
                                                sess.Character.GiftAdd(1689, 150);
                                                sess.Character.Gold += 50000000;
                                                sess.SendPacket(sess.Character.GenerateGold());
                                            }
                                        }
