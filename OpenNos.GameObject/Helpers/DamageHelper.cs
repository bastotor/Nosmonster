...
namespace OpenNos.GameObject.Helpers
{
    public class DamageHelper
    {
            ...
            ref bool onyxWings, bool attackGreaterDistance = false)
        {
            ...
            //Hardcoded monsters raid boss damage
            if (defender.MapMonster?.MonsterVNum == 451)
            {
                totalDamage = 46;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 452)
            {
                totalDamage = 334;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 532)
            {
                totalDamage = 392;
                percentDamage = true;
            }
            ...
            if (defender.MapMonster?.MonsterVNum == 796)
            {
                totalDamage = 200;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 1381)
            {
                totalDamage = 600;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 1500 || defender.MapMonster?.MonsterVNum == 774)
            {
                totalDamage = 338;
                percentDamage = false; //Kentao (false)
            }
            if (defender.MapMonster?.MonsterVNum == 1501 || defender.MapMonster?.MonsterVNum == 1502 || defender.MapMonster?.MonsterVNum == 1503)
            {
                totalDamage = 2000;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 2309 || defender.MapMonster?.MonsterVNum == 2316 || defender.MapMonster?.MonsterVNum == 2357)
            {
                totalDamage = 193;
                percentDamage = false; //Kentao (false)
            }
            if (defender.MapMonster?.MonsterVNum == 2314)
            {
                totalDamage = 3666;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 2315)
            {
                totalDamage = 3948;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 2331)
            {
                totalDamage = 676;
                percentDamage = false; //kentao 
            }
            if (defender.MapMonster?.MonsterVNum == 2332)
            {
                totalDamage = 507;
                percentDamage = true;
            }
            if (defender.MapMonster?.MonsterVNum == 2639)
            {
                totalDamage = 666;
                percentDamage = false;
            }
            if (defender.MapMonster?.MonsterVNum == 2710)
            {
                totalDamage = 100;
                percentDamage = true;
            }
            ...
            if (defender.MapMonster != null)
            {
                ...
                shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageIncreasedtotheEnemy) / 100D;
                if (defender.MapMonster.Monster.Race == 0 && defender.MapMonster.Monster.RaceType == 0) // bastoc
                {
                    shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageIncreasedtothePlant) / 100D;

                }
                if (defender.MapMonster.Monster.Race == 0 && defender.MapMonster.Monster.RaceType == 1)
                {
                    shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageIncreasedtotheAnimal) / 100D;
                }
                if (defender.MapMonster.Monster.Race == 0 && defender.MapMonster.Monster.RaceType == 2)
                {
                    shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageIncreasedtotheUnDead) / 100D;

                }
                if (defender.MapMonster.Monster.Race == 1 && defender.MapMonster.Monster.RaceType == 2)
                {
                    shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageincreasedtotheSmallMonster) / 100D;
                    shellBoostCategory2 += GetShellWeaponEffectValue(ShellWeaponEffectType.DamageincreasedtotheBigMonster) / 100D;
                }
            }
            ...
            attacker.CritChance += GetDefenderBenefitingBuffs(CardType.Critical, (byte)AdditionalTypes.Critical.ReceivingDecreased)[0]; //On the database the value is negative
            ...
            attacker.CritRate += GetAttackerBenefitingBuffs(CardType.Count, (byte)AdditionalTypes.Count.CriticalDamageIncreased)[0];//bastob
            attacker.CritRate -= GetDefenderBenefitingBuffs(CardType.StealBuff, (byte)AdditionalTypes.StealBuff.ReduceCriticalReceivedChance)[0];//bastob
            attacker.CritRate += GetDefenderBenefitingBuffs(CardType.Critical, (byte)AdditionalTypes.Critical.DamageFromCriticalDecreased)[0]; //On the database the value is negative
...
#warning Disabled Critical Rate limit
            ...
            #region Damage reduction //bastoc
            ...
            switch (attacker.AttackType)
            {
                case AttackType.Range:
                    ...
                    {
                        percentageReduction = GetDefenderBenefitingBuffs(CardType.Damage,
                            (byte)AdditionalTypes.Damage.RangedDecreased)[0];
                    }
                ...
                case AttackType.Melee:
                    ...
                    {
                        percentageReduction = GetDefenderBenefitingBuffs(CardType.Damage,
                            (byte)AdditionalTypes.Damage.MeleeDecreased)[0];
                    }
                ...
                case AttackType.Magical:
                    ...
                    {
                        percentageReduction = GetDefenderBenefitingBuffs(CardType.Damage,
                            (byte)AdditionalTypes.Damage.MagicalDecreased)[0];
                    }
            ...
            {
                hitMode = 4;
                defender.Character.ChargeValue = Math.Min(totalDamage, 7000);
            ...
            #region Rigth Buff Way //bastob

            //if (attacker.GetBuff(CardType.BuffInAttack, (byte)AdditionalTypes.BuffInAttack.LGWings) is int[] GeneralBuff)
            //{
            //    if (ServerManager.RandomNumber() <= GeneralBuff[0])
            //    {
            //        defender.AddBuff(new Buff((short)754, attacker.Level), attacker);
            //    }
            //}            

            if (attacker.Character != null && attacker.GetBuffInRW(CardType.BuffInAttack, (byte)AdditionalTypes.BuffInAttack.GeneralInAttack) is List<int[]> Test1)
            {
                foreach (int[] bcard in Test1)
                {
                    if (ServerManager.RandomNumber() <= bcard[0])
                    {
                        defender.AddBuff(new Buff((short)bcard[1], attacker.Level), attacker);
                    }
                }
            }

            if (attacker.Character != null && attacker.GetBuffInRW(CardType.BuffInAttack, (byte)AdditionalTypes.BuffInAttack.PositiveInAttack) is List<int[]> Positive)
            {
                foreach (int[] bcard in Positive)
                {
                    if (ServerManager.RandomNumber() <= bcard[0])
                    {
                        attacker.AddBuff(new Buff((short)bcard[1], defender.Level), defender);
                    }
                }
                //0 Probabilidad
            ...
            //if (defender.GetBuff(CardType.BuffInAttack, (byte)AdditionalTypes.BuffInAttack.GeneralPositiveInFefense) is int[] PositiveD)
            //{
            //    if (ServerManager.RandomNumber() <= PositiveD[0])
            //    {
            //        defender.AddBuff(new Buff((short)PositiveD[1], defender.Level), defender);
            //    }
            //}
            ...
            if (defender.GetBuff(CardType.SummonAct7, (byte)AdditionalTypes.SummonAct7.SummoningFromDefense) is int[] SummonData)
            {
                if (ServerManager.RandomNumber() < SummonData[2])
                {
                    List<MonsterToSummon> list = new List<MonsterToSummon>();

                    for (int i = 0; i < (SummonData[0] == 0 ? ServerManager.RandomNumber(1, 2) : ServerManager.RandomNumber(SummonData[0], SummonData[0] + 1)); i++)
                    {
                        MapCell cell = new MapCell { X = (short)(ServerManager.RandomNumber(-1, 2) + defender.PositionX), Y = (short)(ServerManager.RandomNumber(-1, 2) + defender.PositionY) };
                        list.Add(new MonsterToSummon((short)SummonData[1], cell, null, true, aliveTime: 60, owner: defender, isHostile: true));
                    }
                    EventHelper.Instance.RunEvent(new EventContainer(defender.MapInstance, EventActionType.SPAWNMONSTERS, list));

                    //First Data Numero de monstruos a invocar
                    //Second Data Vnum Monster 
                    //Thrid Data Probabilidad
                }
            }


            /*if (defender.GetBuff(CardType.SummonAct7, (byte)AdditionalTypes.SummonAct7.SummoningFromArmor) is int[] SummonDataArmor) // kentao crash
            {
                if (ServerManager.RandomNumber() < SummonDataArmor[0])
                {
                    List<MonsterToSummon> list = new List<MonsterToSummon>();

                    for (int i = 0; i < (SummonDataArmor[2] == 0 ? ServerManager.RandomNumber(1, 2) : ServerManager.RandomNumber(1, SummonDataArmor[2])); i++) // ici le crash
                    {
                        MapCell cell = new MapCell { X = (short)(ServerManager.RandomNumber(-1, 2) + defender.Character.PositionX), Y = (short)(ServerManager.RandomNumber(-1, 2) + defender.Character.PositionY) };
                        list.Add(new MonsterToSummon((short)SummonDataArmor[1], cell, null, true, aliveTime: 60, owner: defender, isHostile: true));
                    }
                    EventHelper.Instance.RunEvent(new EventContainer(defender.MapInstance, EventActionType.SPAWNMONSTERS, list));

                    //First Data Probalidad
                    //Second Data Vnum Monster 
                    //Thrid Data 0 Siempre spawnea 1
                }
            }*/

            #endregion

            #region DragonSpUpgrade 

            if (attacker.Character?.SpInstance != null && attacker.Character.SpInstance.Upgrade == 20) //fixsp kentao +20
            {
                if (5 > ServerManager.RandomNumber())
                {
                    onyxWings = true;
                }
            }
            ...
            #region Runes, Tattoos and BCards related to it


            if (attacker.Character != null)
            {
                var bcardsRunes = attacker.BCards.Where(x =>
                    x.Type == (byte)CardType.A7Powers1 || x.Type == (byte)CardType.A7Powers2);
            ...
            // I declared a single array for all of this combinations and else, due to I won't be creating multiple variables.
            var mutableArray = new int[4];

            // Sharp Claws and cards that work like that
            mutableArray = attacker.GetBuff(CardType.Critical,
                (byte)AdditionalTypes.Critical.DamageIncreasingPropability);
            if (ServerManager.RandomProbabilityCheck(mutableArray[0]))
            {
                totalDamage += Convert.ToInt32((totalDamage / 100D) * Math.Abs(mutableArray[1]));
            }

            // Arcane Wisdom (Attack Magic) and cards that work like that
            mutableArray = attacker.GetBuff(CardType.Tattoos,
                (byte)AdditionalTypes.Tattoos.OverallAttackIncreasedByMagicDefense);

            totalDamage += Convert.ToInt32((attacker.MagicalDefense / 100D) * mutableArray[0]);
            ...
            #region InvisibleIllusionistIncreaseDamage
            if (attacker.EntityType == EntityType.Player)
            {
                if (attacker.Character.GetBuff(CardType.EffectSummon, (byte)AdditionalTypes.EffectSummon.Illusionist)[0] > 0)
                {
                    totalDamage += attacker.Character.SwitchLevel();
                }
            }
            #endregion

            #region PoliceManRemoveDebuff
            int PoliceMan = GetDefenderBenefitingBuffs(CardType.EffectSummon, (byte)AdditionalTypes.EffectSummon.PoliceMan)[0];
            if (ServerManager.RandomNumber(0, 100) < PoliceMan)
            {
                foreach (Buff b in defender.Character.Buff.Where(s => s.Card.Level <= 4 && s.Card.BuffType == BuffType.Bad))
                {
                    defender.Character.RemoveBuff(b.Card.CardId);
                }
            }
            #endregion

            // basto
             #region RecoveryHPInDefence 
                ...
                && (defender.EntityType == EntityType.Player))
            {
                ...
                if (rec > 0)
                {
                    double tot = (totalDamage * (rec / 100D)) / 15; // kentao (2.5) taux de régène par dégâts recu.
                ...
                && (defender.EntityType == EntityType.Player))
            {
                ...
                if (rec > 0)
                {
                    double tot = (totalDamage * (rec / 100D)) / 50; //kentao (50) taux de % de régène par dégâts recu.
                    defender.MapInstance?.Broadcast(defender.Character.GenerateRc((int)tot));
                    totalDamage -= (int)tot;
                }
            }
            ...
            #endregion
            if (defender.HasBuff(CardType.IncreaseRes, (byte)AdditionalTypes.IncreaseRes.DecreaseTotalDamageBy)) //bastoc
            {
            ...
            #region Apply effect when hitting target //bastob

            // Buff added to attacker from defender
            if (defender.HasBuff(CardType.SecondSPCard, (byte)AdditionalTypes.SecondSPCard.HitAttacker))
            {
                ...
                if (ServerManager.RandomNumber() <= probabilities)
                    attacker.AddBuff(new Buff(buffId, defender.Level), defender);
            }

            // Buff added to defender from attacker
            if (attacker.HasBuff(CardType.ApplyBuffs, (byte)AdditionalTypes.ApplyBuffs.InflictOnAttacking))
            {
                var effect = attacker.GetBuff(CardType.ApplyBuffs, (byte)AdditionalTypes.ApplyBuffs.InflictOnAttacking);

                var probabilities = effect[0];
                var buffId = (short)effect[1];

                if (ServerManager.RandomNumber() <= probabilities)
                    defender.AddBuff(new Buff(buffId, defender.Level), defender);
            }

            // Buff added to attacker from defender
            if (defender.HasBuff(CardType.ApplyBuffs, (byte)AdditionalTypes.ApplyBuffs.InflictOnDefending))
            {
                var effect = defender.GetBuff(CardType.ApplyBuffs, (byte)AdditionalTypes.ApplyBuffs.InflictOnDefending);
            ...
            if (attacker.Character != null)
            {
                ...
                if (defender.MapMonster != null)
                {
                    ...
                    // Damage Vs Sealed Monster //bastob
                    ...
                    if ((attacker.HasBCard(CardType.IncreaseDamageVsMonsterInMap,
                              (byte)AdditionalTypes.IncreaseDamageVsMonsterInMap.IncreaseDamageVsSealledMonster) ||
                          attacker.HasBCard(CardType.SealedLodDamage,
                              (byte)AdditionalTypes.SealedLodDamage.ExtraDamageToVesselsAndGlacernon)) &&
                              vnums.Contains(defender.MapMonster.MonsterVNum))

                    {
                        damageIncrease += (int)(totalDamage * (attacker.GetBuff(CardType.IncreaseDamageVsMonsterInMap,
                            (byte)AdditionalTypes.IncreaseDamageVsMonsterInMap
                                .IncreaseDamageVsSealledMonster)[0] * 0.01D));
                        damageIncrease += (int)(totalDamage * (attacker.GetBuff(CardType.SealedLodDamage,
                            (byte)AdditionalTypes.SealedLodDamage
                                .ExtraDamageToVesselsAndGlacernon)[0] * 0.01D));
                    }
            ...
            if (defender.Character != null)
            {
                ...
                if (bo > 0)
                {
                    var heal = (int)(totalDamage / 10 * (bo * 0.04));
                    if (heal > 0)
                    {
            ...
            if (defender.HasBuff(475)) return 0;//|| defender.HasBuff(767)// bastoc

            if (attacker.HasBuff(CardType.DealMoreDamage, (byte)AdditionalTypes.DealMoreDamage.IncreaseDamagePercent)) //bastoc
            {
                var data = attacker.GetBuff(CardType.DealMoreDamage,
                        (byte)AdditionalTypes.DealMoreDamage.IncreaseDamagePercent)[0];
                var data2 = attacker.GetBuff(CardType.DealMoreDamage,
            ...
            // Increase + decrease TotalDamage by percent
            if (attacker.HasBuff(CardType.Critical, (byte)AdditionalTypes.Critical.DamageIncreasingPropability))
            {
                var buff = attacker.Character.GetBuff(CardType.Critical, (byte)AdditionalTypes.Critical.DamageIncreasingPropability);
                var probability = buff[0];

                if (probability < ServerManager.RandomNumber() || probability >= 100)
                {
                    var percent = buff[1];
                    var finalValue = (int)(totalDamage * (percent / 100D));
            ...
            // Oeil pour Oeil
            if (defender.HasBuff(CardType.Tattoos, (byte)AdditionalTypes.Tattoos.RevertDamageFromPercent))
            {
                var buff = defender.GetBuff(CardType.Tattoos, (byte)AdditionalTypes.Tattoos.RevertDamageFromPercent);
                var probability = buff[0];

                if (probability < ServerManager.RandomNumber() || probability >= 100)
                {
                    var percent = buff[1];
                    int dam = (int)(totalDamage * percent / 100d);
                    totalDamage -= dam;
                    attacker.GetDamage(dam, defender, true);

                    defender.MapInstance.Broadcast(StaticPacketHelper.SkillUsed(realAttacker.UserType, realAttacker.MapEntityId, (byte)realAttacker.UserType, realAttacker.MapEntityId,
                        -1, 0, 0, 0, 0, 0, realAttacker.Hp > 0, (int)(realAttacker.Hp / realAttacker.HPLoad() * 100), dam, 0, 1));

                    defender.Character?.Session?.SendPacket(defender.Character.GenerateStat());
                }
            }

            int[] manaShield = defender.GetBuff(CardType.LightAndShadow,
                    (byte)AdditionalTypes.LightAndShadow.InflictDamageToMP);
            if (manaShield[0] != 0 && hitMode != 4)
            {
                int reduce = totalDamage / 100 * manaShield[0];
                if (defender.Mp < reduce)
                {
                    reduce = defender.Mp;
                    defender.Mp = 0;
                }
                ...
                {
                    defender.DecreaseMp(reduce);
                }
                totalDamage -= reduce;
            }
            if (defender != null)
            {
                if (defender.HasBuff(CardType.TauntSkill, (byte)AdditionalTypes.TauntSkill.ReflectBadEffect)) // smart fix for glad 
                {
                    totalDamage = 0;
                }
            }
