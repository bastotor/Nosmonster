...
namespace OpenNos.Import.Console
{
    public class ImportFactory
    {
        ...
        public void ParseFishingFile(string fileFishDat, ref List<FishingSpotsDto> fishingSpots, ref List<FishInfoDto> fishInfo)
        {
            ...
            using StreamReader fishDatReader = new StreamReader(fileFishDat, Encoding.GetEncoding(1252));
            while ((line = fishDatReader.ReadLine()) != null)
            {
                ...
                switch (split[0])
                {
                    ...
                    case "MAP":
                        ...
                        break;
                    case "POS":
                        fishingSpots.Add(new FishingSpotsDto
                        {
                            MapId = mapId,
                    ...
                    case "ITEM":
                        ...
                        break;
                    case "BASIC":
                        //Seems you can get some items in some cases, I don't really know how that happens
                        fishInfo.Add(new FishInfoDto
                        {
                            FishVNum = short.Parse(split[2]),
                            Probability = short.Parse(split[3]),
        ...
        public void ParseFishing()
        {
            string fileFishDat = $"{_folder}\\fish.dat";
            string fileItemDat = $"{_folder}\\Item.dat";
            string fileItemLang = $"{_folder}\\_code_{ConfigurationManager.AppSettings[nameof(Language)]}_Item.txt";
            Dictionary<string, string> languageKeys = new Dictionary<string, string>();

            List<FishingSpotsDto> fishingSpots = new List<FishingSpotsDto>();
            List<FishInfoDto> fishInfo = new List<FishInfoDto>();

            string line;

            ParseFishingFile(fileFishDat, ref fishingSpots, ref fishInfo);
            ...
            using (StreamReader itemDatReader = new StreamReader(fileItemDat, Encoding.GetEncoding(1252)))
            {
                ...
                while ((line = itemDatReader.ReadLine()) != null)
                {
                    ...
                    if (string.IsNullOrEmpty(currentKey) || !languageKeys.ContainsKey(currentKey) || currentVNum < 10000) // All fishes have a VNum > 10400. Kinda dirty, but will be kinda future proof
                    {
                        continue;
                    }
                    ...
                    {
                        sizeSplit[0] = Convert.ToDecimal(sizeSplit[0].Replace(".", ",")).ToString();
                    }
                    else
                    {
                        sizeSplit[0] = Convert.ToDecimal(sizeSplit[0].Replace(",", ".")).ToString();
                    ...
                    {
                        fish.MinFishLength = float.Parse(sizeSplit[0]);
                        fish.MaxFishLength = float.Parse(sizeSplit[1]);
                    }
                    currentVNum = 0;
            ...
            DAOFactory.FishingSpotsDao.InsertOrUpdateFromList(fishingSpots);
            DAOFactory.FishInfoDao.InsertOrUpdateFromList(fishInfo);
            Logger.Log.Info($"{fishingSpots.Count} fishing spots saved");
            Logger.Log.Info($"{fishInfo.Count} fish info saved");
        }
                            ...
                            case "LINK":
                                ...
                                switch (quest.QuestId)
                                {
                                    ...
                                    case 3331:
                                        ...
                                        break;
                                    case 3374:
                                        quest.NextQuestId = 5478; // first main act5 quest
                                        break;
        ...
        {
            string fileAct_DescDat = $"{_folder}\\act_desc.dat";
            string fileAct_DescLang = $"{_folder}\\_code_{ConfigurationManager.AppSettings[nameof(Language)]}_act_desc.txt";
            List<ActDTO> acts = new List<ActDTO>();
            List<ActPartDTO> actParts = new List<ActPartDTO>();
            using (StreamReader stream = new StreamReader(fileAct_DescDat, Encoding.GetEncoding(1252)))
            {
                string line;
                while ((line = stream.ReadLine()) != null)
                {
                    string[] currentLine = line.Split(' ', '\t');
                    if (currentLine.Length == 3 && currentLine[0] == "A")
                    {
                        acts.Add(new ActDTO
                        {
                            Title = currentLine[3],
                            ActId = Convert.ToByte(currentLine[2]),
                            Scene = (byte)(39 + Convert.ToByte(currentLine[2]))
                        });
                        continue;
                    }

                    else if (currentLine.Length == 5 && currentLine[0] == "DATA")
                    {
                        actParts.Add(new ActPartDTO
                        {
                            ActPartNumber = Convert.ToByte(currentLine[2]),
                            ActPartId = Convert.ToByte(currentLine[4]),
                            ActId = Convert.ToByte(currentLine[3]),
                            MaxTs = Convert.ToByte(currentLine[5]),
                        });
                        continue;
                    }
                }
            ...
            DAOFactory.ActDAO.Insert(acts);
            DAOFactory.ActPartDAO.Insert(actParts);
            Logger.Info(string.Format(Language.Instance.GetMessageFromKey("ACTS_PARTS_PARSED"), actParts.Count));
        }
        ...
        {
            // Recipe for Bleach SP3 quest
            InsertRecipe(1321, 1322, 1, new short[] { 1323, 1, 1324, 1, 1325, 1, 1331, 1, 1322, 1 });

            // Toundra bittoren quest letter
            InsertRecipe(1395, 1398, 1, new short[] { 1390, 1, 1391, 1, 1392, 1, 1393, 1, 1394, 1, 1398, 1 });

            // SP4 quest unfinished spear
            InsertRecipe(1350, 1361, 1, new short[] { 1352, 1, 1353, 1, 1354, 1, 1355, 1, 1356, 1, 1361, 1 });
            ...
            // Amir's  Weapon Specification
            InsertRecipe(349, 1315, 1, new short[] { 2266, 80, 2247, 35, 2241, 5, 1315, 1 });
            InsertRecipe(353, 1315, 1, new short[] { 2232, 80, 2259, 35, 2272, 5, 1315, 1 });
            InsertRecipe(356, 1315, 1, new short[] { 2244, 80, 2228, 35, 2258, 5, 1315, 1 });
            InsertRecipe(352, 1315, 1, new short[] { 402, 1, 2045, 60, 2230, 5, 2238, 3, 2244, 70, 2247, 99, 1315, 1 });
            InsertRecipe(351, 1315, 1, new short[] { 405, 1, 2037, 60, 2237, 3, 2243, 70, 2266, 70, 2274, 99, 1315, 1 });
            InsertRecipe(355, 1315, 1, new short[] { 408, 1, 2049, 60, 2225, 70, 2232, 70, 2236, 3, 2273, 99, 1315, 1 });

            // Amir's Weapon Specification Book Cover
            InsertRecipe(1315, 1316, 1, new short[] { 1312, 10, 1313, 10, 1314, 10, 1316, 1 });

            // Orc Loa Weapons Scroll
            InsertRecipe(4449, 5751, 1, new short[] { 4448, 1, 2416, 300, 2406, 200, 2430, 40, 2432, 1, 2474, 10, 5751, 1 });
            InsertRecipe(4452, 5751, 1, new short[] { 4451, 1, 2416, 300, 2415, 25, 2430, 45, 2432, 1, 2474, 10, 5751, 1 });
            InsertRecipe(4458, 5751, 1, new short[] { 4457, 1, 2416, 450, 2430, 40, 2466, 140, 2413, 10, 2474, 10, 5751, 1 });
            InsertRecipe(4455, 5751, 1, new short[] { 4454, 1, 2416, 300, 2408, 40, 2412, 250, 2413, 10, 2474, 10, 5751, 1 });

            // Orc Loa Secondary Weapons Scroll
            InsertRecipe(4465, 5752, 1, new short[] { 4460, 1, 2408, 135, 2430, 40, 2416, 150, 2413, 5, 5752, 1 });
            InsertRecipe(4468, 5752, 1, new short[] { 4467, 1, 2366, 250, 2406, 25, 2416, 150, 2432, 1, 5752, 1 });
            InsertRecipe(4471, 5752, 1, new short[] { 4470, 1, 2408, 250, 2406, 40, 2416, 150, 2412, 5, 5752, 1 });
            InsertRecipe(4474, 5752, 1, new short[] { 4473, 1, 2411, 250, 2406, 40, 2410, 150, 2413, 10, 5752, 1 });
        ...
        public void ImportMapNpcs()
        {
            ...
            foreach (string[] currentPacket in _packetList.Where(o => o[0].Equals("in") || o[0].Equals("at")))
            {
                ...
                if (currentPacket.Length > 7 && currentPacket[0] == "in" && currentPacket[1] == "2")
                {
                    ...
                    //Need review of this if we want like official game : It works for now but not as labyrinth
                    if (npctest.MapId == 120 && npctest.MapX == 20 && npctest.MapY == 8)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 120 && npctest.MapX == 80 && npctest.MapY == 50)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 120 && npctest.MapX == 50 && npctest.MapY == 119)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 120 && npctest.MapX == 80 && npctest.MapY == 73)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 120 && npctest.MapX == 72 && npctest.MapY == 78)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 120 && npctest.MapX == 43 && npctest.MapY == 125)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 47 && npctest.MapY == 7)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 37 && npctest.MapY == 3)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 63 && npctest.MapY == 67)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 66 && npctest.MapY == 62)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 77 && npctest.MapY == 125)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
                    ...
                    if (npctest.MapId == 121 && npctest.MapX == 79 && npctest.MapY == 114)
                    {
                        teleporter = new TeleporterDTO
                        {
                            Index = 0,
        ...
        public void ImportMaps()
        {
            ...
            void addMap(short originalMapId, short mapId, byte[] mapData)
            {
                string name = "";
                int music = 0;
        ...
        public static void ImportMapType()
        {
            ...
            {
                MapTypeId = (short)MapTypeEnum.Act71,
                MapTypeName = "Act7",
                PotionDelay = 300,
                RespawnMapTypeId = (long)RespawnType.DefaultAct7,
                ReturnMapTypeId = (long)RespawnType.ReturnAct7
            };
        ...
        public static void ImportMapTypeMap()
        {
            ...
            for (int i = 1; i < 3000; i++)
            {
                ...
                else if ((i > 40 && i < 45) || (i > 45 && i < 48) || (i > 99 && i < 102) || (i > 104 && i < 128) || (i > 400 && i < 413))
                {
                    // "act3"
                    mapTypeId = (short)MapTypeEnum.Act3;
                    objectset = true;
                }
                else if ((i >= 260 && i < 264) || (i > 2613 && i < 2621))
                {
                    // "act3.2"
                    mapTypeId = (short)MapTypeEnum.Act32;
                    objectset = true;
                ...
                else if (i > 227 && i < 241 || i == 2603 || i == 2604)
                {
                    // "act6.1"
                    mapTypeId = (short)MapTypeEnum.Act61;
                    objectset = true;
                }
                else if ((i >= 241 && i < 247) || i == 299)
                {
                    // "act6.2"
                    mapTypeId = (short)MapTypeEnum.Act62;
                    objectset = true;
                }
                else if (i > 2627 && i < 2652)
                {
                    // "act7.1"
                    mapTypeId = (short)MapTypeEnum.Act71;
                    objectset = true;
        ...
        public void ImportMonsters()
        {
            ...
            foreach (string[] currentPacket in _packetList.Where(o => o[0].Equals("in") || o[0].Equals("at")))
            {
                ...
                // monster.MonsterVNum == 860 /* remove Broken Red Plate for quests*/) : Example if you need to remove one monster from packet.txt
                {
                    continue;
                }
        ...
        public void ImportNpcMonsters()
        {
            ...
            if (DAOFactory.DropDAO.LoadAll().Count == 0)
            {
                ...
                drops.Add(new DropDTO { ItemVNum = 1012, Amount = 1, MonsterVNum = null, DropChance = 5000, MapTypeId = (short)MapTypeEnum.Act62 });
                ...
                drops.Add(new DropDTO { ItemVNum = 2099, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2102, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2114, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2115, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2116, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                ...
                drops.Add(new DropDTO { ItemVNum = 2206, Amount = 1, MonsterVNum = null, DropChance = 1200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2803, Amount = 1, MonsterVNum = null, DropChance = 200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2804, Amount = 1, MonsterVNum = null, DropChance = 200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2805, Amount = 1, MonsterVNum = null, DropChance = 200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2806, Amount = 1, MonsterVNum = null, DropChance = 200, MapTypeId = (short)MapTypeEnum.Act62 });
                drops.Add(new DropDTO { ItemVNum = 2807, Amount = 1, MonsterVNum = null, DropChance = 500, MapTypeId = (short)MapTypeEnum.Act62 });
                ...
                // Act 7 
                drops.Add(new DropDTO { ItemVNum = 1004, Amount = 1, MonsterVNum = null, DropChance = 500, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1007, Amount = 1, MonsterVNum = null, DropChance = 500, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1010, Amount = 1, MonsterVNum = null, DropChance = 500, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1012, Amount = 1, MonsterVNum = null, DropChance = 8000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1086, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1078, Amount = 1, MonsterVNum = null, DropChance = 500, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1114, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1235, Amount = 1, MonsterVNum = null, DropChance = 150, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1237, Amount = 1, MonsterVNum = null, DropChance = 150, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1238, Amount = 1, MonsterVNum = null, DropChance = 20, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1239, Amount = 1, MonsterVNum = null, DropChance = 150, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1240, Amount = 1, MonsterVNum = null, DropChance = 20, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 1241, Amount = 1, MonsterVNum = null, DropChance = 400, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2098, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2099, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2100, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2101, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2102, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2114, Amount = 1, MonsterVNum = null, DropChance = 1000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2115, Amount = 1, MonsterVNum = null, DropChance = 1000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2116, Amount = 1, MonsterVNum = null, DropChance = 1000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2117, Amount = 1, MonsterVNum = null, DropChance = 1000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2118, Amount = 1, MonsterVNum = null, DropChance = 1000, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2129, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2205, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2206, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2207, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2208, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2282, Amount = 1, MonsterVNum = null, DropChance = 4500, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2283, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2284, Amount = 1, MonsterVNum = null, DropChance = 300, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2285, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 2296, Amount = 1, MonsterVNum = null, DropChance = 100, MapTypeId = (short)MapTypeEnum.Act71 });
                drops.Add(new DropDTO { ItemVNum = 5119, Amount = 1, MonsterVNum = null, DropChance = 50, MapTypeId = (short)MapTypeEnum.Act71 });
            }
        ...
        public void ImportPortals()
        {
            ...
            PortalDTO cylloanPortal1 = new PortalDTO
            {
                SourceMapId = 2526,
                SourceX = 35,
                SourceY = 34,
                DestinationMapId = 228,
                Type = -1,
                DestinationX = 150,
                DestinationY = 42,
                IsDisabled = false
            };
            DAOFactory.PortalDAO.Insert(cylloanPortal1);

            PortalDTO cylloanPortal2 = new PortalDTO
            {
                SourceMapId = 2527,
                SourceX = 35,
                SourceY = 34,
                DestinationMapId = 243,
                Type = -1,
                DestinationX = 70,
                DestinationY = 244,
                IsDisabled = false
            };
            DAOFactory.PortalDAO.Insert(cylloanPortal2);

            PortalDTO backToCylloanShip = new PortalDTO
            {
                SourceMapId = 243,
                SourceX = 66,
                SourceY = 245,
                DestinationMapId = 2527,
                Type = -1,
                DestinationX = 35,
                DestinationY = 34,
                IsDisabled = false
            };
            DAOFactory.PortalDAO.Insert(backToCylloanShip);

            PortalDTO KremToCavern = new PortalDTO
            {
                SourceMapId = 21,
                SourceX = 96,
                SourceY = 150,
            ...
            DAOFactory.PortalDAO.Insert(KremToCavern);

            PortalDTO KremToCavern1 = new PortalDTO
            {
                SourceMapId = 21,
                SourceX = 78,
                SourceY = 150,
            ...
            DAOFactory.PortalDAO.Insert(KremToCavern1);

            PortalDTO Act4Demon = new PortalDTO
            {
                SourceMapId = 153,
                SourceX = 135,
                SourceY = 171,
            ...
            DAOFactory.PortalDAO.Insert(Act4Demon);
        ...
        public static void ImportRespawnMapType()
        {
            List<RespawnMapTypeDTO> respawnmaptypemaps = new List<RespawnMapTypeDTO>
            {
                ...
                new RespawnMapTypeDTO
                {
                    RespawnMapTypeId = (long)RespawnType.ReturnAct7,
                    DefaultMapId = 2700,
                    DefaultX = 57,
                    DefaultY = 81,
                    Name = "ReturnAct7"
