        ...
        public void BankAction(GboxPacket gboxPacket)
        {
            ...
            switch (gboxPacket.Type)
            {
                case BankActionType.Deposit:
                    ...
                    {
                        if (gboxPacket.Amount > Session.Character.Gold) //bastodupl
                        {
...
#pragma warning disable 4014
                DiscordWebhookHelper.DiscordEventSay4($"TchatGroup:  Personnage: {Session.Character.Name} - MAP: {Session.CurrentMapInstance?.Map.Name} - Message:  {groupSayPacket.Message} ");
        ...
        public void Guri(GuriPacket guriPacket)
        {
            if (guriPacket != null)
            {
                ...
                else if (guriPacket.Type == 99999)
                {
                    if (ServerManager.Instance.EventInWaiting && BattleROYA2.Map.Sessions.Count() < BattleROYA2.MaxAllowedPlayers)
                    {
                        ServerManager.Instance.TeleportOnRandomPlaceInMap(Session, BattleROYA2.Map.MapInstanceId);
                        Group group = new Group
                        {
                        ...
                        if (Session.Character.Group != null)
                        {
                            BattleROYA2.AddGroup(group);
                        }
                        else
                        {
                            group.Sessions.Add(Session);
                        }
                        Session.SendPacket("bsinfo 0 5 28 0");
                        Session.SendPacket("esf 0");
                        BattleROYA2.AddGroup(group);
                    }
                }//basto
                        ...
                        if (session != null && !session.Character.IsChangingMapInstance)
                        {
                            if (Session.Character.IsFriendOfCharacter(guriPacket.User))
                            {
                                if (session.CurrentMapInstance?.MapInstanceType == MapInstanceType.BaseMapInstance)
                                {
                                    ...
                                    if (session.Character.MapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act71 && Session.Character.Level < 88 && Session.Character.HeroLevel < 10))
                                    {
                                        Session.SendPacket(
                                          Session.Character.GenerateSay(
                                              Language.Instance.GetMessageFromKey("CANT_USE_THAT"), 10));
        ...
        public void Say(SayPacket sayPacket)
        {
            ...
            else
            {
                ...
                if (Session.Character.Authority == AuthorityType.GameSage)
                {
                    ...
                    message = $"[HELPER {Session.Character.Name}]: " + message;
                }
                if (Session.Character.Authority == AuthorityType.Administrator)
                {
                    ...
                    message = $"[FOUNDER {Session.Character.Name}]: " + message;
                }
...
#pragma warning disable 4014
            DiscordWebhookHelper.DiscordEventSay($"GlobalChat:  Personnage: {Session.Character.Name} - MAP: {Session.CurrentMapInstance?.Map.Name} - Message:  {message} ");
        }
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            if (ServerManager.Instance.Configuration.AutoLootEnable)
            {
                if (Session.Character.LockCode != null)
                {
                    ...
                    Session.SendPacket(Session.Character.GenerateSay($"lock", 12));
                    Session.SendPacket(Session.Character.GenerateSay($"lock", 12));
                }
                else
                {
                    ...
                    Session.SendPacket(Session.Character.GenerateSay($"unlock", 12));
                    Session.SendPacket(Session.Character.GenerateSay($"unlock", 12));
                }
            }
            else
            ...
            if (ServerManager.Instance.Configuration.SceneOnCreate
                && Session.Character.GeneralLogs.CountLinq(s => s.LogType == "Connection") < 5)
            {
            ...
            void Events()
            {
                ...
                {
                    Session.SendPacket(Session.Character.GenerateSay($"Event Activated: Doble Experiencia XP Fairy!", 12));
                    eventcurrent = true;
                }
                if (ServerManager.Instance.Configuration.DoubleExpe == true)
                {
                    Session.SendPacket(Session.Character.GenerateSay($"Event Activated: Doble Experience!", 12));
                    eventcurrent = true;
                }
                if (ServerManager.Instance.Configuration.DoubleRaidBox == true)
                {
                    Session.SendPacket(Session.Character.GenerateSay($"Event Activated: Doble box raid!", 12));
                    eventcurrent = true;
                }
                ...
                {
                    Session.SendPacket(Session.Character.GenerateSay($"There are no active events.", 12));
                }
            }
            ...
            Session.SendPacket($"msg 3 =========NosMonster Rate=========");
...
#pragma warning restore 618
            ...
            Session.SendPacket(Session.Character.GenerateSay($"Welcome To NosMOnster", 12));
            Session.SendPacket(Session.Character.GenerateSay($"====================", 11));
            Session.SendPacket(Session.Character.GenerateSay($"Discord: https://discord.gg/uHjj5y2REB", 12));
            Session.SendPacket(Session.Character.GenerateSay($"Site web: http://www.nosmonster.fr", 12));
            Session.SendPacket(Session.Character.GenerateSay($"For all donations: Ask only Discord", 12));
            Session.SendPacket(Session.Character.GenerateSay($"Please read the rules!", 12));
            Session.SendPacket(Session.Character.GenerateSay($"====================", 11));
...

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"{Session.Character.Name} <- Welcome To NosMonster",
                    Type = MessageType.Shout
                });

            }

            /*if (Session.Account.Authority == AuthorityType.TGM)

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"The Trial Game Master -> {Session.Character.Name} <- has just connected",
                    Type = MessageType.Shout
                });
...

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"The Game Master -> {Session.Character.Name} <- has just connected",
                    Type = MessageType.Shout
                });

            }

            if (Session.Account.Authority == AuthorityType.SGM)

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"The Super Game Master -> {Session.Character.Name} <- has just connected",
                    Type = MessageType.Shout
                });
...

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"The Developer -> {Session.Character.Name} <- has just connected",
                    Type = MessageType.Shout
                });
...

            {
                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage()
                {
                    ...
                    SourceCharacterId = Session.Character.CharacterId,
                    SourceWorldId = ServerManager.Instance.WorldId,
                    Message = $"The Founder -> {Session.Character.Name} <- has just connected",
                    Type = MessageType.Shout
                });
            ...
            ////Buff de level de famille avec niveau

            if (Session.Character.Family != null)
            {
                if (Session.Character.Family.FamilyLevel >= 5)
                {
                    if (!Session.Character.Buff.ContainsKey(116))
                    {
                        Session.Character.AddStaticBuff(new StaticBuffDTO { CardId = 116 });
                    }
                    else
                    {
                        return;
                ...
                {
                    if (!Session.Character.Buff.ContainsKey(117))
                    {
                        Session.Character.AddStaticBuff(new StaticBuffDTO { CardId = 117 });
                    }
                    else
                    {
                        return;
                ...
                {
                    if (!Session.Character.Buff.ContainsKey(661))
                    {
                        Session.Character.AddStaticBuff(new StaticBuffDTO { CardId = 661 });
                    }
                    else
                    {
                        return;
            ...
            //    Session.Character.AddBuff(new Buff(116, Session.Character.Level), Session.Character.BattleEntity);
            //    Session.Character.AddBuff(new Buff(117, Session.Character.Level), Session.Character.BattleEntity);
            //    Session.Character.AddBuff(new Buff(661, Session.Character.Level), Session.Character.BattleEntity);
...
#pragma warning disable 4014
                    DiscordWebhookHelper.DiscordEventSay3($"WHisper:  Personnage: {Session.Character.Name} - MAP: {Session.CurrentMapInstance?.Map.Name} Autre: {receiver.CharacterId} - Message:  {message} "); // la calote de tes mort 
