...
namespace OpenNos.Handler
{
    public class InventoryPacketHandler : IPacketHandler
    {
        ...
        public bool HasBuff2(CardType type, byte subtype) => BattleEntity.HasBuff(type, subtype);
        ...
        public void Deposit(DepositPacket depositPacket)
        {
            if (depositPacket != null)
            {
                ...
                if (item.ItemVNum == 2333)
                {
                    Session.SendPacket("msg 4 You can't deposit this Item");
                    return;
                }
                                                              ...
                                                              != Session.Character.CharacterId)
                            {
                                ...
                                lock (targetSession.Character.Inventory)
                                {
                                    lock (Session.Character.Inventory)
                                    {
                                        ...
                                        var newbackpack = targetSession.Character.HaveNewBackpack() ? 1 : 0;
                                        ...
                                        if (Session.Character.ExchangeInfo.Validated && targetExchange.Validated)
                                        {
                                            ...
                                            if (targetExchange != null && targetExchange.Confirmed && Session.Character.ExchangeInfo.Confirmed)
                                            {
                                                ...
                                                var @continue = true;
                                                var goldmax = false;
                                                if (!Session.Character.Inventory.EnoughPlaceV2(targetExchange.ExchangeList, ((Session.Character.HaveBackpack() ? 1 : 0) * 12) + ((Session.Character.HaveNewBackpack() ? 1 : 0) * 60)))
                                                {
                                                    @continue = false;
                                                }
                                ...
                                if (mapItemInstance.Item.ItemType == ItemType.Map || mapItem.IsQuest)
                                {
                                    if (mapItem is MonsterMapItem)
                                    {
                                        ...
                                        if (mapItem.IsQuest)
                                        {
                                            ...
                                            Session.Character.IncrementQuests(QuestType.Collect5, mapItem.ItemVNum);
...
#pragma warning disable 4014
                                            DiscordWebhookHelper.DiscordPickUp($"PickUp:  Personnage: {Session.Character.Name} - Id: {inv.Id} - ItemVNum: {inv.ItemVNum} - Qantité: {amount} ");
                                        }
        ...
        public void MoveEquipment(MvePacket mvePacket)
        {
            if (mvePacket != null)
            {
                lock (Session.Character.Inventory)
                {
                    ...
                    if (mvePacket.DestinationInventoryType == InventoryType.Bazaar || mvePacket.DestinationInventoryType == InventoryType.Etc || mvePacket.DestinationInventoryType == InventoryType.Main || mvePacket.DestinationInventoryType == InventoryType.Warehouse)
                    {
                        Session.SendPacket("msg 4 Action blocked");
                        return;
                    }

                    if (mvePacket.DestinationSlot > 48 + ((Session.Character.HaveBackpack() ? 1 : 0) * 12) + ((Session.Character.HaveNewBackpack() ? 1 : 0) * 60))
                    {
                        return;
                    }
        ...
        public void MoveItem(MviPacket mviPacket)
        {
            ...
            if (mviPacket != null)
            {
                ...
                {
                    // check if the destination slot is out of range
                    if (mviPacket.DestinationSlot > 48 + ((Session.Character.HaveBackpack() ? 1 : 0) * 12) + ((Session.Character.HaveNewBackpack() ? 1 : 0) * 60))
                    {
                        return;
                    }
        ...
        public void PutItem(PutPacket putPacket)
        {
            ...
            if (putPacket == null || Session.Character.HasShopOpened)
            {
             ...
             if (Session.Character.LastDropRequests > 30) //bastoban
             {
...
#pragma warning disable 4014
                            DiscordWebhookHelper.DiscordDrop($"DropItem:  Personnage: {Session.Character.Name} - Map:{Session.CurrentMapInstance.Map.MapId} - Item: {droppedItem.ItemVNum} - Amount: {droppedItem.Amount}");
                            Session.CurrentMapInstance?.Broadcast(
                    ...
                    if (specialist != null)
                    {
                        if (specialist.Rare != -2)
                        {
                            ...
                            if (specialist.Item.EquipmentSlot == EquipmentType.Sp)
                            {
                                ...
                                while (run)
                                {
                                    run = specialist.UpgradeSp(Session, UpgradeProtection.Protected);
                                }
                            }
                        }
                        else
                    ...
                    if (specialist != null)
                    {
                        if (specialist.Rare != -2)
                        {
                            if (specialist.Item.EquipmentSlot == EquipmentType.Sp)
                            {
                                ...
                                while (run)
                                {
                                    run = specialist.PerfectSP(Session);
                                }
                            }
                        }
                        else
        ...
        public void UseItem(UseItemPacket useItemPacket)
        {
            ...
            {
                if (itemInstance?.IsBattleRoyal == false && Session.CurrentMapInstance.MapInstanceType == MapInstanceType.BattleRoyalInstance)
                {
                    Session.SendPacket("modal 1 tu ne peux pas utiliser ton item");
                    return;
                }
                if (itemInstance?.IsBattleRoyal == true && Session.CurrentMapInstance.MapInstanceType != MapInstanceType.BattleRoyalInstance)
                {
                    Session.SendPacket("modal 1 tu ne peux pas utiliser ton item");
                    return;
                }
        ...
        public void Wear(WearPacket wearPacket)
        {
                ...
                == null)
            {
                ...
                if (inv?.Item != null)
                {
                    if (inv.IsBattleRoyal == false && Session.CurrentMapInstance.MapInstanceType == MapInstanceType.BattleRoyalInstance)
                    {
                        Session.SendPacket("modal 1 tu ne peux pas T'équiper de ton item");
                        return;
                    }
                    if (inv.IsBattleRoyal == true && Session.CurrentMapInstance.MapInstanceType != MapInstanceType.BattleRoyalInstance)
                    {
                        Session.SendPacket("modal 1 tu ne peux pas T'équiper de ton item");
                        return;
                    }
        ...
        public void Withdraw(WithdrawPacket withdrawPacket)
        {
            if (withdrawPacket != null)
            {
                ...
                Logger.LogUserEvent("WAREHOUSE_WITHDRAW_LOG_PACKET", Session.GenerateIdentity(), $"Packet string: {withdrawPacket.OriginalContent.ToString()}");
        ...
        private void ChangeSp()
        {
            ...
            if (sp != null)
            {
                ...
                Parallel.ForEach(ServerManager.GetAllSkill(), skill =>
                {
                    ...
                    if (sp.ItemVNum == 4485 || sp.ItemVNum == 4437 || sp.ItemVNum == 4416 || sp.ItemVNum == 4532 || sp.ItemVNum == 8378)
                    {
                        morphUpdate = 30;
                    }
...
#pragma warning disable 4014
            DiscordWebhookHelper.DiscordEchange($"Echange:  Personnage: {sourceSession.Character.Name} - Message:  {data} ");
