...
namespace OpenNos.Handler
{
    /*public class BazaarPacketHandler : IPacketHandler
    {
        ...
        public DateTime LastAddToInventory { get; set; }
        ...
        public void BuyBazaar(CBuyPacket cBuyPacket)
        {
            ...
            if (Session.Character.BazarRequests > 20) //bastoban
             {
            ...
            if (!Session.Character.VerifiedLock)
            {
              ...
              //  string[] packetsplit = cBuyPacket.Split(' ');
        ...
        public void RefreshPersonalBazarList(CSListPacket csListPacket)
        {
            ...
            SpinWait.SpinUntil(() => !ServerManager.Instance.InBazaarRefreshMode);
            //Session.SendPacket(Session.Character.GenerateRCSList(csListPacket)); //kentao modif
        }
        ...
        public void SellBazaar(CRegPacket cRegPacket)
        {
                ...
                if (Session.Character.LastBazaarInsert.AddSeconds(5) > DateTime.Now)
                {
                 ...
                 if (cRegPacket.Inventory != 0 && cRegPacket.Inventory != 1 && cRegPacket.Inventory != 2 && cRegPacket.Inventory != 4) //bastoban
                {
                     foreach (var team in ServerManager.Instance.Sessions.Where(s => s.Account.Authority >= AuthorityType.SGM))
                     {
                ...
                if (it == null || !it.Item.IsSoldable || !it.Item.IsTradable || it.IsBound || it.IsBattleRoyal)
                {
                    return;
                }
