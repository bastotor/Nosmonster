    ...
    internal class ScriptedInstancePacketHandler : IPacketHandler
    {
        ...
        public void GenerateRaid(MkRaidPacket mkRaidPacket)
        {
            if (Session.Character.Group?.Raid != null && Session.Character.Group.IsLeader(Session))
            {
                ...
                {
                    if ((Session.Character.Group.SessionCount > 0)
                    && Session.Character.Group.Sessions.All(s => s.CurrentMapInstance == Session.CurrentMapInstance))
                    {
                        ...
                        if (ServerManager.Instance.Act6DemonStat.IsBossErenia == true && Session.Character.MapId == 236)
                        {
                            Session.SendPacket("msg 4 The Audience with Erenia is not started");
                            return;
                        }
                       

                        if (ServerManager.Instance.Act6AngelStat.IsBossZenas == true && Session.Character.MapId == 232)
                        {
                            Session.SendPacket("msg 4 The Audience with Zenas is not started");
                            return;
                        }
        ...
        {
            var ts12 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1526);
            var ts21 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1562);
            var ts31 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1630);
            var ts40 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1687);
            var ts51 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1742);
            var ts55 = Session.Character.Quests.FirstOrDefault(q => q.QuestId == 1751);
