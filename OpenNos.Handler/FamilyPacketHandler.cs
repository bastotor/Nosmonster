...
namespace OpenNos.Handler
{
    public class FamilyPacketHandler : IPacketHandler
    {
        ...
        public ItemInstance BlacklistedItem;
...
#pragma warning disable 4014
                DiscordWebhookHelper.DiscordEventSay2($"TchatFafa:  Personnage: {Session.Character.Name} - MAP: {Session.CurrentMapInstance?.Map.Name} - Message:  {msg} "); // la calote de tes mort 
                ...
                Parallel.ForEach(ServerManager.Instance.Sessions.ToList(), session =>
                {
                        ...
                        && session.Character.Family?.FamilyId == Session.Character.Family?.FamilyId)
                    {
                        ...
                        {
                            if (Session.Account.Authority != AuthorityType.GameMaster && !Session.Character.InvisibleGm)
                            {
        ...
        public void FamilyDeposit(FDepositPacket fDepositPacket)
        {
            ...
            if (BlacklistedItem.ItemVNum == 2333)
            {
        ...
        [Packet("%Familydismiss", "%FamilyKick","%Rejetdefamille")]
        ...
        [Packet("%Aviso", "%Notice", "%Avertissement")]
